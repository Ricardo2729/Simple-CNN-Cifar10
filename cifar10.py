...
# -*- coding: utf-8 -*-
"""
Created on Fri Sep 27 10:07:07 2019
...
# from Net.Vgg19Net import *
# from Net.ResNet import *

model = Vgg16_Net().to(device)
#model = Vgg19_Net().to(device)
...
#torch.save(model, '../../model_hub/cifar10/model_907.pkl')   #保存模型
#model = torch.load('../../model_hub/cifar10/model_907.pkl')  #加载模型
...
def viz(module, input):
    ...
    x = input[0][0].cpu()
    # print(x.device)
    #把主要显示分格限制在4*4，剩余的以1*n的形式显示，可忽略，也可自己再调整
    # num = min(4,x.size()[0])
    for i in range(x.size()[0]):
        # plt.subplot(1,num, i+1) #分格显示
        plt.xticks([])  #关闭x刻度
        plt.yticks([])  #关闭y刻度
        plt.axis('off') #关闭坐标轴
